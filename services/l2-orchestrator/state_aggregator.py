...
Phase 12e: Temporal Firewall — Reflexive DT → ABDT State Aggregation
"""
import json
import os
import sys
...
SIGNIFICANT_EVENT_THRESHOLD = 5.0
...
def ensure_topics(bootstrap_servers: str, client_id: str = "ctt-bootstrap"):
    ...
    try:
        ...
        print(f"[StateAggregator] ✅ Created topics: {[t.name for t in CTT_TOPICS]}")
    except TopicAlreadyExistsError:
        print(f"[StateAggregator] ℹ️ Topics already exist, skipping creation")
    except Exception as e:
        print(f"[StateAggregator] ⚠️ Topic bootstrap warning (non-fatal): {e}")
    finally:
        if admin:
            admin.close()
...
class StateAggregator:
    def __init__(self):
        ...
        if HAS_KAFKA:
            try:
                ensure_topics(KAFKA_BOOTSTRAP, client_id="state-aggregator-bootstrap")
        ...
        try:
            ...
            for agent in data:
                snap = TickSnapshot(
                    ...
                    speed_mps=agent.get("speed_mps", 0.0),
    ...
    def _aggregate(self, agent_id: str, ticks: List[TickSnapshot]) -> AggregatedObservation:
        ...
        shift_count = sum(1 for i in range(1, len(decarbonized)) if decarbonized[i] != decarbonized[i-1])
        ...
        return AggregatedObservation(
            ...
            co2_emitted_g=(co2_end - co2_start) * 1000.0,
            nox_emitted_g=sum(t.current_nox_g_km for t in ticks) / 1000.0,
