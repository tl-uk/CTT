...
Phase 12e: ABDT Agent State Cache — Temporal Decoupling Layer
"""
import json
import os
import sys
...
def ensure_topics(bootstrap_servers: str, client_id: str = "ctt-bootstrap"):
    ...
    try:
        ...
        print(f"[ABDTCache] ℹ️ Topics already exist, skipping creation")
    except Exception as e:
        print(f"[ABDTCache] ⚠️ Topic bootstrap warning (non-fatal): {e}")
    finally:
        if admin:
            admin.close()
...
class ABDTAgentCache:
    def __init__(self):
        ...
        if HAS_KAFKA:
            ...
            print(f"[ABDTCache] ⚠️  kafka-python not installed")
